...
from src.customer.customer_check import (
...
class Customers(Cog):
    ...
    async def verify_subscription(
        self, ctx: Context, subscription_id: Optional[str]
    ):
        member = get(self.guild.members, id=ctx.author.id)

        if not subscription_id:
            return await ctx.send(
                "**You have not provided a subscription id.**"
            )
